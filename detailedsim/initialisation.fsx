...
let initParksPositions = 
    ...
    let leslie = {createAgent "leslie" 6 0 20 with RoleOf=Some (Gatekeeper(parks.ID))}
    let april = {createAgent "april" 7 0 20 with RoleOf=Some (Monitor(parks.ID))}
    [parks ; ron ; leslie; april]

let initBrooklynPositions = 
    let ray = {createAgent "ray" 8 0 20 with RoleOf=Some (Head(brooklyn.ID))}
    let terry = {createAgent "terry" 9 0 20 with RoleOf=Some (Gatekeeper(brooklyn.ID))}
    let amy = {createAgent "amy" 10 0 20 with RoleOf=Some (Monitor(brooklyn.ID))}
    [brooklyn ; ray ; terry; amy]

let initHier = [offices; mike; dan] @ initParksPositions @ initBrooklynPositions

let initParks = initMemberAgents parksNames parks ((getLatestId initHier) + 1) 0 20
let initBrooklyn = initMemberAgents brooklynNames brooklyn ((getLatestId initParks) + 1) 0 20
let otherAgents = initAgents agentNames ((getLatestId initBrooklyn) + 1) 0 20
